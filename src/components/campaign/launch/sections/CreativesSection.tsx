...
export default function CreativesSection({ state }: Props) {
  ...
  return (
    <section id="section-creatives" className="bg-white rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="divide-y divide-[#E8ECF3]">
        {config.creatives.map((creative) => (
          <div key={creative.localId} className="px-6 py-5">
                ...
                {creative.file ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => {
                          const isOpen = imagePickerMode?.creativeId === creative.localId && imagePickerMode.mode === 'stock';
                          setImagePickerMode(isOpen ? null : { creativeId: creative.localId, mode: 'stock' });
                          if (!isOpen) {
                            const q = creative.headline || config.campaign.name || 'advertising';
                            setStockQuery(q);
                            handleStockSearch(q);
                          }
                        }}
                        className={`flex-1 flex items-center justify-center gap-1.5 py-3 border-2 border-dashed rounded-lg transition-all cursor-pointer bg-transparent ${
                          imagePickerMode?.creativeId === creative.localId && imagePickerMode.mode === 'stock'
                            ? 'border-emerald-300 text-emerald-500 bg-emerald-50/30'
                            : 'border-gray-200 text-gray-400 hover:text-emerald-500 hover:border-emerald-300 hover:bg-emerald-50/30'
                        }`}
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        <span className="text-[11px] font-medium">Stock Photos</span>
                      </button>
                    </div>
                      ...
                      <div className="border border-gray-200 rounded-lg overflow-hidden">
                        <div className="flex gap-2 p-2 bg-gray-50 border-b border-gray-100">
                          <input
                            ...
                            className="flex-1 px-3 py-1.5 border border-gray-200 rounded-md text-sm text-gray-900 outline-none focus:border-emerald-400"
                            onKeyDown={(e) => { if (e.key === 'Enter') handleStockSearch(stockQuery); }}
                            autoFocus
                          />
                          <button
                            onClick={() => handleStockSearch(stockQuery)}
                            disabled={stockLoading}
                            className="px-3 py-1.5 text-xs font-medium text-white bg-emerald-500 hover:bg-emerald-600 rounded-md transition-colors border-none cursor-pointer disabled:opacity-50"
                          >
                            {stockLoading ? '...' : 'Search'}
                          </button>
                        </div>
                        {stockResults.length > 0 && (
                          <div className="grid grid-cols-4 gap-1 p-2 max-h-48 overflow-y-auto">
                            {stockResults.map((photo) => (
                              <button
                                key={photo.id}
                                onClick={() => handleStockSelect(creative.localId, photo)}
                                className="relative aspect-square rounded-md overflow-hidden border-2 border-transparent hover:border-emerald-400 transition-all cursor-pointer bg-gray-100 p-0"
                              >
                                ...
                                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent px-1 py-0.5">
                                  <span className="text-[8px] text-white truncate block">{photo.photographer}</span>
                                </div>
                              </button>
                            ))}
                          </div>
                        ...
                        {stockLoading && (
                          <div className="p-4 text-center text-xs text-gray-400">
                            <div className="w-4 h-4 border-2 border-gray-200 border-t-emerald-500 rounded-full animate-spin mx-auto mb-1" />
                            Searching...
                          </div>
                        )}
                      </div>
