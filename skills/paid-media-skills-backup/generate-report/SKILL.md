...
- A **template type** indicating which report structure to use
...
### executive-summary
Lead with KPI overview and AI summary. Include channel breakdown, budget pacing,
and strategic recommendations. Generate **4 sections minimum**:
1. Executive Summary (`summary`) — high-level narrative of campaign performance
2. Key Performance Metrics (`metrics`) — top-line KPIs with period-over-period changes
3. Channel Breakdown (`table`) — performance by channel (spend, ROAS, conversions)
4. Strategic Recommendations (`insights`) — AI-driven recommendations for stakeholders

### campaign-performance
Detailed campaign-level analysis with performance trends. Generate **5-6 sections**:
1. Performance Overview (`summary`) — narrative summary of campaign health
2. Performance Trend (`chart`) — daily/weekly performance trend data
3. Campaign Performance Table (`table`) — campaign-level metrics (spend, impressions, clicks, CTR, CPA, ROAS)
4. Budget Utilization (`metrics`) — budget pacing and utilization metrics
5. ROAS Analysis (`comparison`) — ROAS by campaign or channel vs targets
6. Conversion Funnel (`chart`) — funnel visualization (impressions → clicks → conversions)

### creative-analysis
Creative format breakdown with fatigue tracking and optimization recommendations.
Generate **5 sections**:
1. Format Performance (`table`) — performance by creative format (video, image, carousel, etc.)
2. Top Creatives (`table`) — top-performing creatives with key metrics
3. Fatigue Status (`metrics`) — creative fatigue scores and trend indicators
4. A/B Test Results (`comparison`) — test variant performance comparisons
5. Refresh Recommendations (`insights`) — AI recommendations for creative refresh

### full-report
Comprehensive report combining all sections. Generate **8+ sections**:
1. Executive Summary (`summary`)
2. Key Performance Metrics (`metrics`)
3. Channel Performance (`table`)
4. Campaign Details (`table`)
5. Performance Trends (`chart`)
6. Audience Insights (`insights`)
7. Creative Analysis (`table`)
8. Budget & ROAS Analysis (`comparison`)
9. Strategic Recommendations (`insights`)
...
8. **Match the template type.** When a template type is specified, follow the
   section structure for that template. Include all recommended sections for the
   chosen template type.
9. **Use realistic data.** Generate plausible metric values, trends, and insights
   that reflect real-world paid media campaign patterns.
